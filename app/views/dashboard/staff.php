...
<div class="main">
  <?php
    ...
    $pendingSummaryDefaults = ['count' => 0, 'amount' => 0];
    $pendingSummary = array_merge($pendingSummaryDefaults, $pending_summary ?? []);
    ...
    $patientCount = (int)($patient_count ?? 0);
    $doctorCount = (int)($doctor_count ?? 0);
  ...
  <?php
    $topbar_title = 'Clinic Operations Dashboard';
  ...
  <div class="row g-3 my-4">
    ...
    <div class="col-sm-6 col-xl-3">
      <div class="card-soft border border-info-subtle">
        <div class="d-flex justify-content-between align-items-center">
          <div>
            <small class="text-muted text-uppercase fw-semibold">Confirmed / Done</small>
    ...
    <div class="col-sm-6 col-xl-3">
      <div class="card-soft border border-warning-subtle">
        <div class="d-flex justify-content-between align-items-center">
          <div>
            <small class="text-muted text-uppercase fw-semibold">Pending Payments</small>
            <h3 class="mt-2 mb-0"><?= (int)$stats['pending_payments']; ?></h3>
            <small class="text-muted">₱ <?= number_format((float)$pendingSummary['amount'], 2); ?> outstanding</small>
          </div>
          <div class="fs-30">💰</div>
        </div>
      </div>
    ...
    <div class="col-sm-6 col-xl-3">
      <div class="card-soft border border-danger-subtle">
        <div class="d-flex justify-content-between align-items-center">
          <div>
            <small class="text-muted text-uppercase fw-semibold">Cancelled Today</small>
            <h3 class="mt-2 mb-0"><?= (int)$stats['cancelled_today']; ?></h3>
          </div>
          <div class="fs-30">⚠️</div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="row g-3 mb-4">
    <div class="col-md-4">
      <div class="card-soft h-100">
        <small class="text-muted text-uppercase">Patients on record</small>
        <h3 class="mt-2 mb-0"><?= $patientCount; ?></h3>
        <p class="text-muted small mb-0">Across the entire clinic.</p>
      </div>
    </div>
    <div class="col-md-4">
      <div class="card-soft h-100">
        <small class="text-muted text-uppercase">Active Doctors</small>
        <h3 class="mt-2 mb-0"><?= $doctorCount; ?></h3>
        <p class="text-muted small mb-0">Ready to receive bookings.</p>
      </div>
    </div>
    <div class="col-md-4">
      <div class="card-soft h-100">
        ...
        <p class="text-muted small mb-0">Lifetime clinic volume.</p>
      </div>
    </div>
  </div>
    ...
    <div class="col-12 col-xl-8">
      <div class="card-soft h-100">
        <div class="d-flex justify-content-between align-items-center mb-3">
          <div>
            <h5 class="mb-0">Today's Clinic Schedule</h5>
            <small class="text-muted"><?= htmlspecialchars($today_label ?? date('l, F d, Y')); ?></small>
          </div>
          <a href="<?= base_url(); ?>/appointments" class="btn btn-outline-secondary btn-sm">View all</a>
        </div>
              ...
              <?php if (!empty($todaySchedule)): ?>
                <?php foreach ($todaySchedule as $visit): ?>
                  <?php
                    ...
                    $appointmentId = (int)($visit['appointment_id'] ?? 0);
                    $modalLabel = ($visit['patient'] ?? 'Patient') . ' • ' . ($visit['time'] ?? 'Schedule');
                  ?>
                  <tr>
                    <td>
                      <div class="fw-semibold"><?= htmlspecialchars($visit['time'] ?? '—'); ?></div>
                      <small class="text-muted"><?= htmlspecialchars($visit['date_label'] ?? ''); ?></small>
                    </td>
                    <td><?= htmlspecialchars($visit['patient'] ?? 'Patient'); ?></td>
                    <td><?= htmlspecialchars($visit['doctor'] ?? 'Doctor'); ?></td>
                    <td>
                    ...
                    <td>
                      <span class="badge <?= $statusBadge; ?>" style="text-transform:capitalize;"><?= htmlspecialchars($statusKey); ?></span>
                    </td>
                      ...
                      <div class="d-flex justify-content-end gap-2">
                        <?php if ($appointmentId > 0): ?>
                          <?php if (($visit['status'] ?? '') !== 'completed'): ?>
                            <a href="<?= base_url(); ?>/appointments/<?= $appointmentId; ?>/complete" class="btn btn-sm btn-success">Mark Done</a>
                            <button type="button"
    ...
    <div class="col-12 col-xl-4">
      <div class="card-soft mb-3">
        <div class="d-flex justify-content-between align-items-center mb-2">
          <h6 class="mb-0">Pending Payments Follow-up</h6>
              ...
              <div class="list-group-item px-0">
                <div class="d-flex justify-content-between align-items-start">
                  <div>
                    ...
                    <small class="text-muted">with <?= htmlspecialchars($pay['doctor'] ?? 'Doctor'); ?></small><br>
                    <small class="text-muted"><?= htmlspecialchars($pay['slot_label'] ?? 'TBD'); ?></small>
                  </div>
                  <div class="text-end">
        ...
        <?php else: ?>
          <p class="text-muted mb-0">All invoices are settled. 🎉</p>
        <?php endif; ?>
        <div class="mt-3 text-end">
          <a href="<?= base_url(); ?>/payments/records" class="btn btn-outline-primary btn-sm">Open payment records</a>
        </div>
      </div>
      ...
      <div class="card-soft">
        ...
        <div class="d-grid gap-2 mt-3">
          <a href="<?= base_url(); ?>/appointments/staff_add" class="btn btn-primary">Create Appointment</a>
          <a href="<?= base_url(); ?>/patients" class="btn btn-outline-secondary">Patient Directory</a>
    ...
    <div class="col-12 col-xl-7">
      <div class="card-soft h-100">
        <div class="d-flex justify-content-between align-items-center mb-3">
          ...
          <small class="text-muted">Grouped by payment status</small>
        </div>
        <div class="table-responsive">
          <table class="table table-hover align-middle mb-0">
            <thead class="table-light">
            ...
            <tbody>
              ...
              <?php else: ?>
                <tr>
                  <td colspan="5" class="text-center text-muted py-4">No scheduled appointments in the next week.</td>
                </tr>
              <?php endif; ?>
            </tbody>
          </table>
    ...
    <div class="col-12 col-xl-5">
      <div class="card-soft h-100">
        <div class="d-flex justify-content-between align-items-center mb-3">
          ...
          <small class="text-muted">Latest 6 visits</small>
        </div>
              ...
              <div class="list-group-item px-0">
                <div class="d-flex justify-content-between align-items-start">
                  <div>
                    ...
                    <small class="text-muted">with <?= htmlspecialchars($completed['doctor'] ?? 'Doctor'); ?></small><br>
